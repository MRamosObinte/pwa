...
@Component({
  selector: 'app-pago-anticipo-detalle',
  templateUrl: './pago-anticipo-detalle.component.html'
})
export class PagoAnticipoDetalleComponent implements OnInit {

  @Input() totales: any = {};
  @Input() configAutonumericReales: AppAutonumeric;
  @Input() listadoPagosAnticipos: Array<CarFunPagosSaldoAnticipoTO> = new Array();
  @Input() accion: string = null;
  @Output() enviarAccion = new EventEmitter();

  public objetoSeleccionado: CarFunPagosSaldoAnticipoTO = new CarFunPagosSaldoAnticipoTO();
  ...
  completarTablaPagos() {
    this.cobrarTodosCheck = true;
    this.totales.anticipo = 0;
    if (this.listadoPagosAnticipos) {
      for (let i = 0; i < this.listadoPagosAnticipos.length; i++) {
        let data = this.listadoPagosAnticipos[i];
        this.listadoPagosAnticipos[i]['saldo'] = this.listadoPagosAnticipos[i].antValor;
        this.listadoPagosAnticipos[i].antValor = data.antValor ? data.antValor : 0;
        if (data.antValor === data['saldo']) {
          this.listadoPagosAnticipos[i]['cobrar'] = true;
        } else {
          this.listadoPagosAnticipos[i]['cobrar'] = false;
        }
        this.totales.anticipo = this.mathRound2(this.totales.anticipo) + this.mathRound2(data.antValor);
        this.totales.diferencia = this.mathRound2(this.totales.pago) - (this.mathRound2(this.totales.forma) + this.mathRound2(this.totales.anticipo));
      }
    ...
    columnas.push(
      ...
      },
      {
        ...
        suppressKeyboardEvent: (params) => {
          if (params.editing) { return (this.utilService.desplazarTablaArribaAbajo(params.event.keyCode, params.node.rowIndex, contexto.listadoPagosAnticipos) || this.utilService.desplazarseInputIzquierdaDerecha(params.event.keyCode)) }
        },
        editable: true,
        cellClassRules: {
