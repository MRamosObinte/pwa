...
export class ListaCobrosListadoService {
  ...
  listarCarFunCobrosTO(parametro, contexto, empresaSelect) {
    this.api.post("todocompuWS/carteraWebController/getCarFunCobrosTO", parametro, empresaSelect)
      .then(respuesta => {
        if (respuesta && respuesta.extraInfo) {
          if (respuesta.extraInfo.length > 1) {
            contexto.despuesDeListarCarFunCobrosTO(respuesta.extraInfo);
          } else {
            this.toastr.warning(LS.MSJ_NO_DATA, 'Aviso');
            contexto.cargando = false;
          }
        } else {
          contexto.despuesDeListarCarFunCobrosTO([]);
  ...
  imprimirCarFunCobrosTO(parametros, contexto, empresaSelect) {
    this.archivoService.postPDF("todocompuWS/carteraWebController/generarReporteCarFunCobrosDetalle", parametros, empresaSelect)
      .then(data => {
        if (data._body.byteLength > 0) {
          this.utilService.descargarArchivoPDF('listadoCarFunCobros.pdf', data);
        } else {
  ...
  exportarCarFunCobrosTO(parametros, contexto, empresaSelect) {
    this.archivoService.postExcel("todocompuWS/carteraWebController/exportarReporteCarFunCobrosDetalle", parametros, empresaSelect)
      .then(data => {
        if (data) {
          this.utilService.descargarArchivoExcel(data._body, "listadoCarFunCobros_");
        } else {
  ...
  imprimirCarFunCobrosPorLote(parametros, contexto, empresaSelect) {
    this.archivoService.postPDF("todocompuWS/carteraWebController/generarReportePorLoteCobros", parametros, empresaSelect)
      .then(data => {
        if (data._body.byteLength > 0) {
          this.utilService.descargarArchivoPDF('listadoCarFunCobros.pdf', data);
        } else {
  ...
  obtenerCobrosPK(listaCobros: Array<CarFunCobrosTO>): Array<String> {
    let listaCobrosPK = Array();
    let cobros = new CarFunCobrosTO();
    for (let item of listaCobros) {
      cobros.cobNumeroSistema = item.cobNumeroSistema;
      listaCobrosPK.push(cobros.cobNumeroSistema);
    }
    return listaCobrosPK;
  }
