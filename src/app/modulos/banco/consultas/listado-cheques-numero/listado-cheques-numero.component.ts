...
@Component({
  selector: 'app-listado-cheques-numero',
  templateUrl: './listado-cheques-numero.component.html',
  styleUrls: ['./listado-cheques-numero.component.css']
})
export class ListadoChequesNumeroComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.listaEmpresas = this.route.snapshot.data['listaChequesNumero'];
  ...
  listadoChequesPorNumero(form: NgForm) {
  ...
  imprimirChequeCobrar() {
    if (this.utilService.verificarPermiso(LS.ACCION_IMPRIMIR, this, true)) {
      this.cargando = true;
      let parametros = { listadoChequesNumero: this.listadoChequeTO };
      this.archivoService.postPDF("todocompuWS/bancoWebController/imprimirReporteChequesNumero", parametros, this.empresaSeleccionada)
        .then(data => {
          if (data._body.byteLength > 0) {
            this.utilService.descargarArchivoPDF('listadoChequesNumero_' + this.utilService.obtenerHorayFechaActual() + '.pdf', data);
          } else {
  ...
  exportarChequeCobrar() {
    if (this.utilService.verificarPermiso(LS.ACCION_EXPORTAR, this, true)) {
      this.cargando = true;
      let parametros = { listadoCheques: this.listadoChequeTO, tipoCheque: LS.ETIQUETA_NUMERO };
      this.archivoService.postExcel("todocompuWS/bancoWebController/exportarReporteCheques", parametros, this.empresaSeleccionada)
        .then(data => {
          if (data) {
            this.utilService.descargarArchivoExcel(data._body, "listadoChequesNumero_");
          } else {
