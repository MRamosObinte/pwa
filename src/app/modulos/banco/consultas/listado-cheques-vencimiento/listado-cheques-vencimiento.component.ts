...
@Component({
  selector: 'app-listado-cheques-vencimiento',
  templateUrl: './listado-cheques-vencimiento.component.html',
  styleUrls: ['./listado-cheques-vencimiento.component.css']
})
export class ListadoChequesVencimientoComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.listaEmpresas = this.route.snapshot.data['listaChequesVencimiento'];
  ...
  listadoChequesPorVencimiento(form: NgForm) {
  ...
  imprimirChequeCobrar() {
    if (this.utilService.verificarPermiso(LS.ACCION_IMPRIMIR, this, true)) {
      this.cargando = true;
      let parametros = { listadoChequesVencimiento: this.listadoChequeTO };
      this.archivoService.postPDF("todocompuWS/bancoWebController/imprimirReporteChequesVencimiento", parametros, this.empresaSeleccionada)
        .then(data => {
          if (data._body.byteLength > 0) {
            this.utilService.descargarArchivoPDF('listadoChequesVencimiento_' + this.utilService.obtenerHorayFechaActual() + '.pdf', data);
          } else {
  ...
  exportarChequeCobrar() {
    if (this.utilService.verificarPermiso(LS.ACCION_EXPORTAR, this, true)) {
      this.cargando = true;
      let parametros = { listadoCheques: this.listadoChequeTO, tipoCheque: LS.ETIQUETA_VENCIMIENTO };
      this.archivoService.postExcel("todocompuWS/bancoWebController/exportarReporteCheques", parametros, this.empresaSeleccionada)
        .then(data => {
          if (data) {
            this.utilService.descargarArchivoExcel(data._body, "listadoChequesVencimiento_");
          } else {
  ...
  generarAtajosTeclado() {
    ...
    this.atajoService.add(new Hotkey(LS.ATAJO_IMPRIMIR, (event: KeyboardEvent): boolean => {
      let element: HTMLElement = document.getElementById('btnImprimirChequeCobrar') as HTMLElement;
      element ? element.click() : null;
      return false;
    }))
