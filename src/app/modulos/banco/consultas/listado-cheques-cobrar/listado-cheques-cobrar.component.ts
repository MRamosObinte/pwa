...
@Component({
  selector: 'app-listado-cheques-cobrar',
  templateUrl: './listado-cheques-cobrar.component.html',
  styleUrls: ['./listado-cheques-cobrar.component.css']
})
export class ListadoChequesCobrarComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.listaEmpresas = this.route.snapshot.data['listaChequesCobrar'];
  ...
  listadoChequesPorCobrar(form: NgForm) {
  ...
  imprimirChequeCobrar() {
    if (this.utilService.verificarPermiso(LS.ACCION_IMPRIMIR, this, true)) {
      this.cargando = true;
      let parametros = { listadoChequesCobrar: this.listadoChequeTO };
      this.archivoService.postPDF("todocompuWS/bancoWebController/imprimirReporteChequesCobrar", parametros, this.empresaSeleccionada)
        .then(data => {
          if (data._body.byteLength > 0) {
            this.utilService.descargarArchivoPDF('listadoChequesCobrar_' + this.utilService.obtenerHorayFechaActual() + '.pdf', data);
          } else {
  ...
  exportarChequeCobrar() {
    if (this.utilService.verificarPermiso(LS.ACCION_EXPORTAR, this, true)) {
      this.cargando = true;
      let parametros = { listadoCheques: this.listadoChequeTO, tipoCheque: LS.ETIQUETA_PENDIENTES };
      this.archivoService.postExcel("todocompuWS/bancoWebController/exportarReporteCheques", parametros, this.empresaSeleccionada)
        .then(data => {
          if (data) {
            this.utilService.descargarArchivoExcel(data._body, "listadoChequesCobrar_");
          } else {
