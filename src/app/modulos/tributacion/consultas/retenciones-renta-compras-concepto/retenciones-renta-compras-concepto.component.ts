...
@Component({
  selector: 'app-retenciones-renta-compras-concepto',
  templateUrl: './retenciones-renta-compras-concepto.component.html',
  styleUrls: ['./retenciones-renta-compras-concepto.component.css']
})
export class RetencionesRentaComprasConceptoComponent implements OnInit {
  ...
  constructor(
    ...
    private retencionesRentaComprasConcepto: RetencionesRentaComprasConceptoService,
  ...
  ngOnInit() {
    this.constantes = LS;
    this.listaEmpresas = this.route.snapshot.data['retencionesRentaComprasConcepto'];
  ...
  listaDeRetencionesTipoDocumento(form?: NgForm) {
    ...
    if (form && form.valid) {
      ...
      this.retencionesRentaComprasConcepto.obtenerAnexoListaRetencionesRentaTO(parametro, this, LS.KEY_EMPRESA_SELECT);
    } else {
  ...
  //#region [AG-GRID] 
  iniciarAgGrid() {
    this.columnDefs = this.retencionesRentaComprasConcepto.generarColumnas();
