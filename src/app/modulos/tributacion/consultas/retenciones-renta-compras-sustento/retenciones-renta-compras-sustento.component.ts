...
@Component({
  selector: 'app-retenciones-renta-compras-sustento',
  templateUrl: './retenciones-renta-compras-sustento.component.html',
  styleUrls: ['./retenciones-renta-compras-sustento.component.css']
})
export class RetencionesRentaComprasSustentoComponent implements OnInit {
  ...
  constructor(
    ...
    private retencionesRentaComprasSustento: RetencionesRentaComprasSustentoService,
  ...
  ngOnInit() {
    this.constantes = LS;
    this.listaEmpresas = this.route.snapshot.data['retencionesRentaComprasSustento'];
  ...
  listaDeRetencionesTipoDocumento(form?: NgForm) {
    ...
    if (form && form.valid) {
      ...
      let parametro = {
        ...
        pOrden: LS.TAG_SUSTENTO.toUpperCase()
      }
      this.cargando = true;
      this.filasTiempo.iniciarContador();
      this.retencionesRentaComprasSustento.obtenerAnexoListaRetencionesRentaTO(parametro, this, LS.KEY_EMPRESA_SELECT);
    } else {
  ...
  //#region [AG-GRID] 
  iniciarAgGrid() {
    this.columnDefs = this.retencionesRentaComprasSustento.generarColumnas();
  ...
  mostrarContextMenu(data, event) {
    ...
    if (this.mayorAuxiliarSeleccionado.retSustentotributario) {
