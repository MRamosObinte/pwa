...
@Component({
  selector: 'app-retenciones-renta-compras-tipo-proveedor',
  templateUrl: './retenciones-renta-compras-tipo-proveedor.component.html',
  styleUrls: ['./retenciones-renta-compras-tipo-proveedor.component.css']
})
export class RetencionesRentaComprasTipoProveedorComponent implements OnInit {
  ...
  constructor(
    ...
    private retencionesRentaComprasTipoProveedor: RetencionesRentaComprasTipoProveedorService,
  ...
  ngOnInit() {
    this.constantes = LS;
    this.listaEmpresas = this.route.snapshot.data['retencionesRentaComprasTipoProveedor'];
  ...
  listaDeRetencionesRentaProveedor(form?: NgForm) {
    this.filtrarRapido();
    if (form && form.valid) {
      this.limpiarResultado();
      let parametro = {
        ...
        pOrden: LS.TAG_TIPO_PROVEEDOR
      }
      this.cargando = true;
      this.filasTiempo.iniciarContador();
      this.retencionesRentaComprasTipoProveedor.obtenerAnexoListaRetencionesRentaTO(parametro, this, LS.KEY_EMPRESA_SELECT);
    } else {
  ...
  imprimirRetencionesRentaComprasTipoProveedor() {
  ...
  exportarRetencionesRentaComprasTipoProveedor() {
    if (this.utilService.verificarPermiso(LS.ACCION_EXPORTAR, this, true)) {
      ...
      this.archivoService.postExcel("todocompuWS/anexosWebController/exportarRetencionesRentaCompras", parametros, this.empresaSeleccionada)
        .then(data => {
          if (data) {
            this.utilService.descargarArchivoExcel(data._body, "ListadoRetencionesRentaComprasTipoProveedor_");
          } else {
  ...
  //#region [AG-GRID] 
  iniciarAgGrid() {
    this.columnDefs = this.retencionesRentaComprasTipoProveedor.generarColumnas();
