...
export class CostosPiscinaMultipleService {
  ...
  listarCostosPiscinaMultiple(parametro, contexto, empresaSelect) {
    this.api.post("todocompuWS/produccionWebController/modificarCorridaActivoSeleccionMultiple", parametro, empresaSelect)
      .then(respuesta => {
        if (respuesta && respuesta.extraInfo) {
          contexto.despuesListarCostosPiscinaMultiple(respuesta.extraInfo);
        } else {
  ...
  exportarCostosPiscinaMultiple(parametro, contexto, empresaSelect) {
    this.archivoService.postExcel("todocompuWS/produccionWebController/exportarReporteCostoPiscinaMultiple", parametro, empresaSelect)
      .then(data => {
        (data) ? this.utilService.descargarArchivoExcel(data._body, "ListaCostosPiscinaMultiple_") : this.toastr.warning(LS.MSJ_ERROR_EXPORTAR, LS.TAG_AVISO);
        contexto.cargando = false;
