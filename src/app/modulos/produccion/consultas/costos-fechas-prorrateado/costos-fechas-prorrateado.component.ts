...
@Component({
  selector: 'app-costos-fechas-prorrateado',
  templateUrl: './costos-fechas-prorrateado.component.html',
  styleUrls: ['./costos-fechas-prorrateado.component.css']
})
export class CostosFechasProrrateadoComponent implements OnInit {
  ...
  public listadoCostosFechasProrrateado: Array<any> = null;
  ...
  constructor(
    ...
    private consumosMensualesService: CostosFechasProrrateadoService,
  ...
  ngOnInit() {
    ...
    this.listaEmpresas = this.route.snapshot.data['costosFechaProrrateado'];
  ...
  limpiarResultado() {
    this.listadoCostosFechasProrrateado = null;
  ...
  listadoCostos(form: NgForm, agrupadoPor) {
    if (this.consumoService.verificarPermiso(LS.ACCION_CONSULTAR, this.empresaSeleccionada, true)) {
      ...
      if (form && form.valid && formularioTocado) {
        ...
        this.consumosMensualesService.listarCostoPorFechaProrrateado(parametro, this, LS.KEY_EMPRESA_SELECT);
      } else {
  ...
  despuesListarCostoPorFechaProrrateado(data) {
    if (data) {
      if (data.datos[0][0] === " " && data.datos[0][1] === "") {
        data.datos.shift();
      }
      this.retornoTo = data;
      this.listadoCostosFechasProrrateado = data.datos;
  ...
  exportar() {
    if (this.utilService.verificarPermiso(LS.ACCION_EXPORTAR, this, true)) {
      ...
      let parametros = {
        ...
        datos: this.listadoCostosFechasProrrateado,
        codigoSector: this.sectorSeleccionado.secCodigo,
        fechaInicio: this.utilService.formatearDateToStringDDMMYYYY(this.fechaDesde),
        fechaFin: this.utilService.formatearDateToStringDDMMYYYY(this.fechaHasta),
      };
      this.consumosMensualesService.exportarReportePorFechaProrrateado(parametros, this, this.empresaSeleccionada);
    }
  }
  ...
  //#region [AG-GRID] 
  iniciarAgGrid() {
    this.columnDefs = this.consumosMensualesService.generarColumnas(this.columnasDinamicas);
