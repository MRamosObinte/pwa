...
export class CostosProductosProcesosService {
 ...
 */
  listarCostosProductosProcesos(parametro, contexto, empresaSelect) {
    this.api.post("todocompuWS/produccionWebController/modificarCorridaActivo", parametro, empresaSelect)
      .then(respuesta => {
        if (respuesta && respuesta.extraInfo) {
          contexto.despuesListarCostosProductosProcesos(respuesta.extraInfo);
        } else {
  ...
  exportarCostosProductosProcesos(parametro, contexto, empresaSelect) {
    this.archivoService.postExcel("todocompuWS/produccionWebController/exportarReporteCostosProductosProcesos", parametro, empresaSelect)
      .then(data => {
        (data) ? this.utilService.descargarArchivoExcel(data._body, "ListaCostosProductosProcesos_") : this.toastr.warning(LS.MSJ_ERROR_EXPORTAR, LS.TAG_AVISO);
        contexto.cargando = false;
