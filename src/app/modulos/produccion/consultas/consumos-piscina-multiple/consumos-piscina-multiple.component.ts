...
@Component({
  selector: 'app-consumos-piscina-multiple',
  templateUrl: './consumos-piscina-multiple.component.html',
  styleUrls: ['./consumos-piscina-multiple.component.css']
})
export class ConsumosPiscinaMultipleComponent implements OnInit {
  ...
  public listaMultiplePiscinaCorrida: Array<MultiplePiscinaCorrida> = new Array();
  ...
  constructor(
    ...
    private consumoPiscinaMultipleService: ConsumosPiscinaMultipleService,
  ...
  ngOnInit() {
    ...
    this.listaEmpresas = this.route.snapshot.data['consumosPiscinaMultiple'];
  ...
  listadoConsumos(form: NgForm, agrupadoPor) {
    if (this.consumoService.verificarPermiso(LS.ACCION_CONSULTAR, this.empresaSeleccionada, true)) {
      ...
      if (form && form.valid && formularioTocado) {
        let parametro = {
          empresa: this.empresaSeleccionada.empCodigo,
          multiplePiscinaCorrida: this.listaMultiplePiscinaCorrida,
          proceso: "CONSUMO",
          agrupadoPor: agrupadoPor,
        };
        this.filasTiempo.iniciarContador();
        this.consumoPiscinaMultipleService.listarConsumosPiscinaMultiple(parametro, this, LS.KEY_EMPRESA_SELECT);
      } else {
  ...
  despuesDeListarConsumosPiscinaMultiple(data) {
    if (data) {
  ...
  ejecutarAccion(data) {
    ...
    if (data) {
      ...
      var indexTemp = this.listaMultiplePiscinaCorrida.findIndex(item => item.corrida === this.multiplePiscinaCorrida.corrida);
      this.listaMultiplePiscinaCorrida = this.listaMultiplePiscinaCorrida.filter((val, i) => i != indexTemp);
  ...
  exportar() {
    if (this.utilService.verificarPermiso(LS.ACCION_EXPORTAR, this, true)) {
      ...
      this.consumoPiscinaMultipleService.exportarConsumosPiscinaMultiple(parametros, this, this.empresaSeleccionada);
    }
  }
  ...
  agregarMultiplePiscinaCorrida() {
    ...
    if (this.empresaSeleccionada.empCodigo === this.empresaAsignada.empCodigo) {
      if (!this.listaMultiplePiscinaCorrida.find(item => item.corrida === (this.corridaSeleccionada ? this.corridaSeleccionada.corNumero : ''))) {
        ...
        let listaTemporal = [... this.listaMultiplePiscinaCorrida];
        listaTemporal.unshift(multiplePiscinaCorrida);
        this.listaMultiplePiscinaCorrida = listaTemporal;
        this._refreshGrid();
      } else {
  ...
  limpiarCorridasSeleccionadas() {
    this.listaMultiplePiscinaCorrida = [];
    this.empresaAsignada = new PermisosEmpresaMenuTO();
    this.limpiarResultado();
    this._refreshGrid();
  }
