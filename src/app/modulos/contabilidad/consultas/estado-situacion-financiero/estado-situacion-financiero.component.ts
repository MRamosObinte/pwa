...
@Component({
  selector: 'app-estado-situacion-financiero',
  templateUrl: './estado-situacion-financiero.component.html',
  styleUrls: ['./estado-situacion-financiero.component.css']
})
export class EstadoSituacionFinancieroComponent implements OnInit {
  public listaResultado: Array<ConBalanceGeneralTO> = [];
  public listaSectores: Array<PrdListaSectorTO> = [];
  public listaEmpresas: Array<PermisosEmpresaMenuTO> = [];
  public empresaSeleccionada: PermisosEmpresaMenuTO;
  public estadoSituacionFinSeleccionado: ConBalanceGeneralTO = new ConBalanceGeneralTO();
  ...
  constructor(
    ...
    private estadoSituacionFinService: EstadoSituacionFinancieroService,
  ...
  ngOnInit() {
    ...
    this.listaEmpresas = this.route.snapshot.data["estadoSituacionFinanciera"];
  ...
  despuesDeListarPeriodos(listadoPeriodos) {
    ...
    if (this.listadoPeriodos.length > 0) {
      ...
      this.fecha = new Date(this.periodoSeleccionado.perHasta);
  ...
  consultarMayorAuxiliar() {
    if (this.estadoSituacionFinSeleccionado.bgCuenta && this.estadoSituacionFinSeleccionado.bgCuenta.length === this.tamanioEstructura) {
      let fechaInicioMes = this.utilService.obtenerPrimerDiaDelMes(this.fecha);
      this.objetoMayorAuxiliarDesdeFuera = {
        empresa: this.empresaSeleccionada,
        codigoCuentaDesde: this.estadoSituacionFinSeleccionado.bgCuenta,
        cuentaDetalle: this.estadoSituacionFinSeleccionado.bgDetalle.trim(),
  ...
  generarAtajosTeclado() {
    this.atajoService.add(new Hotkey(LS.ATAJO_MOSTRAR_OCULTAR_FILTROS, (): boolean => {
      let element: HTMLElement = document.getElementById('btnActivarEstadoSituacionFinanciera') as HTMLElement;
      element ? element.click() : null;
      return false;
    }))
    this.atajoService.add(new Hotkey(LS.ATAJO_BUSCAR, (): boolean => {
      let element: HTMLElement = document.getElementById('btnBuscarEstadoSituacionFinanciera') as HTMLElement;
      element ? element.click() : null;
      return false;
    }))
    this.atajoService.add(new Hotkey(LS.ATAJO_IMPRIMIR, (): boolean => {
      let element: HTMLElement = document.getElementById('btnImprimirEstadoSituacionFinanciera') as HTMLElement;
      element ? element.click() : null;
      return false;
    }))
    this.atajoService.add(new Hotkey(LS.ATAJO_EXPORTAR, (): boolean => {
      let element: HTMLElement = document.getElementById('btnExportarEstadoSituacionFinanciera') as HTMLElement;
      element ? element.click() : null;
      return false;
    }))
  ...
  generarOpciones() {
    let isValido = this.listaResultado.length > 0 ? this.estadoSituacionFinSeleccionado.bgCuenta.length === this.tamanioEstructura : false;
    this.opciones = [
  ...
  listarSituacionFinanciera(form: NgForm, ocultarDetalleCuenta, ocultarBiolog) {
    this.cargando = true;
    let formularioTocado = this.utilService.establecerFormularioTocado(form);
    if (formularioTocado && form && form.valid) {
      this.limpiarResultado();
      let parametro = {
        ocultar: ocultarBiolog,
        ocultarDetalle: ocultarDetalleCuenta,
      ...
      this.ocultarOpciones = ocultarDetalleCuenta;
      this.filasTiempo.iniciarContador();
      this.estadoSituacionFinService.listarFunBalanceGeneralTO(parametro, this, LS.KEY_EMPRESA_SELECT);
    } else {
  ...
  despuesDeListarFunBalanceGeneralTO(data) {
    this.iniciarAgGrid();
  ...
  imprimirEstadoSituacionFinanciera() {
    if (this.utilService.verificarPermiso(LS.ACCION_IMPRIMIR, this, true)) {
      this.cargando = true;
      let parametros = {
        usuarioEmpresaReporteTO: this.empresaSeleccionada.empCodigo,
        listConBalanceGeneralNecTO: this.listaResultado,
        codigoSector: this.sectorSeleccionado ? this.sectorSeleccionado.secCodigo : '',
        fechaHasta: this.utilService.formatoStringSinZonaHorariaYYYMMDD(this.fecha)
      }
      this.archivoService.postPDF("todocompuWS/contabilidadWebController/generarReporteBalanceGeneralTO", parametros, this.empresaSeleccionada)
        .then(data => {
          if (data._body.byteLength > 0) {
            this.utilService.descargarArchivoPDF('listadoEstadoSituacionFinanciera.pdf', data);
          } else {
  ...
  exportarEstadoSituacionFinanciera() {
    if (this.utilService.verificarPermiso(LS.ACCION_EXPORTAR, this, true)) {
      this.cargando = true;
      let parametros = {
        fecha: this.utilService.formatoStringSinZonaHorariaYYYMMDD(this.fecha),
        sector: this.sectorSeleccionado ? this.sectorSeleccionado.secCodigo : '',
        listConBalanceGeneralTO: this.listaResultado
      };
      this.archivoService.postExcel("todocompuWS/contabilidadWebController/exportarReporteEstadoSituacionFinanciera", parametros, this.empresaSeleccionada)
        .then(data => {
          if (data) {
            this.utilService.descargarArchivoExcel(data._body, "estadoSituacionFinanciera_");
          } else {
  ...
  //#region [AG-GRID] 
  iniciarAgGrid() {
    this.columnDefs = this.estadoSituacionFinService.generarColumnas(this);
  ...
  filaFocused(event) {
    ...
    this.estadoSituacionFinSeleccionado = fila ? fila.data : null;
  }

  ejecutarAccion(data) {
    this.estadoSituacionFinSeleccionado = data;
    this.consultarMayorAuxiliar();
  }
  ...
  mostrarContextMenu(data, event) {
    this.estadoSituacionFinSeleccionado = data;
    if (this.estadoSituacionFinSeleccionado.bgCuenta.length === this.tamanioEstructura) {
