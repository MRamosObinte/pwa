...
import { RhListaFormaPagoBeneficioSocialTO } from '../../../../entidadesTO/rrhh/RhListaFormaPagoBeneficioSocialTO';
import { NgForm } from '@angular/forms';
import { SectorService } from '../../../produccion/archivos/sector/sector.service';
import { PrdListaSectorTO } from '../../../../entidadesTO/Produccion/PrdListaSectorTO';
import { RhFormaPagoBeneficioSocialTO } from '../../../../entidadesTO/rrhh/RhFormaPagoBeneficioSocialTO';
...
@Component({
  selector: 'app-forma-pago-beneficios',
  templateUrl: './forma-pago-beneficios.component.html'
})
export class FormaPagoBeneficiosComponent implements OnInit {
  ...
  public objetoSeleccionado: RhListaFormaPagoBeneficioSocialTO = new RhListaFormaPagoBeneficioSocialTO();
  public listadoFormasPagoBeneficio: Array<RhListaFormaPagoBeneficioSocialTO> = new Array();
  public formaPago: RhFormaPagoBeneficioSocialTO = new RhFormaPagoBeneficioSocialTO();
  ...
  ngOnInit() {
    this.listaEmpresas = this.route.snapshot.data['formaPagoBeneficioSocial'];
    this.innerWidth = window.innerWidth;
    this.isScreamMd = this.innerWidth <= 576 ? false : true;
    this.empresaSeleccionada = this.utilService.seleccionarEmpresa(this.listaEmpresas);
    this.listaEmpresas ? this.cambiarEmpresaSeleccionada() : null;
    this.formaPagoBeneficiosService.definirAtajosDeTeclado();
    this.iniciarAgGrid();
  }
  ...
  resetearFormulario() {
    ...
    this.accion = null;
    this.listadoFormasPagoBeneficio.length === 0 ? this.activar = false : null;
  ...
  cambiarEmpresaSeleccionada() {
    ...
    this.buscarFormasPagoBeneficios(false);
  }
  ...
  limpiarResultado() {
    ...
    this.vistaFormulario = false;
    this.listadoFormasPagoBeneficio = new Array();
  }
  ...
  verificarPermiso(mostraMensaje): boolean {
    return this.formaPagoBeneficiosService.verificarPermiso(this.accion, this.empresaSeleccionada, mostraMensaje);
  }

  nuevaFormasPagoBeneficios() {
    this.formaPago = new RhFormaPagoBeneficioSocialTO();
  ...
  imprimirFormasPagoBeneficios() {
    if (this.utilService.verificarPermiso(LS.ACCION_IMPRIMIR, this, true)) {
      this.cargando = true;
      let parametros = {
        lista: this.listadoFormasPagoBeneficio
      };
      this.archivoService.postPDF("todocompuWS/rrhhWebController/generarReporteRhFormaPagoBeneficiosTO", parametros, this.empresaSeleccionada)
        .then(data => {
          if (data && data._body && data._body.byteLength > 0) {
            this.utilService.descargarArchivoPDF("ListadoFormaPagoBeneficiosSociales_" + this.utilService.obtenerHorayFechaActual() + ".pdf", data);
          } else {
  ...
  exportarFormasPagoBeneficios() {
    if (this.utilService.verificarPermiso(LS.ACCION_EXPORTAR, this, true)) {
      this.cargando = true;
      let parametros = {
        lista: this.listadoFormasPagoBeneficio
      };
      this.archivoService.postExcel("todocompuWS/rrhhWebController/exportarRhFormaPagoBeneficioSocial", parametros, this.empresaSeleccionada)
        .then(data => {
          if (data && data._body && data._body.byteLength > 0) {
            this.utilService.descargarArchivoExcel(data._body, "ListadoFormaPagoBeneficiosSociales_");
          } else {
  ...
  consultarFormaPago() {
    ...
    this.formaPago = new RhFormaPagoBeneficioSocialTO(this.objetoSeleccionado);
    this.vistaFormulario = true;
  }
  ...
  construirCuenta(objetoSeleccionado: RhListaFormaPagoBeneficioSocialTO): ConCuentasTO {
  ...
  inactivar(estado) {
    let parametros;
    let accionRpta: string;
    if (!estado) {
      this.accion = LS.ACCION_ACTIVAR;
      accionRpta = "A";
      parametros = {
        title: LS.ACCION_ACTIVAR,
        texto: LS.MSJ_PREGUNTA_ACTIVAR + "<br> " + LS.RRHH_FORMA_PAGO_BENEFICIOS + ": " + this.objetoSeleccionado.fpDetalle,
        type: LS.SWAL_QUESTION,
      ...
      parametros = {
        ...
        texto: LS.MSJ_PREGUNTA_INACTIVAR + "<br> " + LS.RRHH_FORMA_PAGO_BENEFICIOS + ": " + this.objetoSeleccionado.fpDetalle,
        type: LS.SWAL_QUESTION,
        confirmButtonText: LS.LABEL_ACEPTAR,
        cancelButtonText: LS.LABEL_CANCELAR
      };
    ...
    this.utilService.generarSwallConfirmacionHtml(parametros).then(respuesta => {
      if (respuesta) {//Si presiona CONTABILIZAR
        ...
        let objeto = new RhFormaPagoBeneficioSocialTO(this.objetoSeleccionado);
        objeto.usrEmpresa = LS.KEY_EMPRESA_SELECT;
        let parametro = {
          rhFormaPagoBeneficioSocialTO: objeto,
          accion: 'M',
          accionRpta: accionRpta
        }
        this.formaPagoBeneficiosService.accionFormasPagoBeneficios(parametro, this, LS.KEY_EMPRESA_SELECT);
  ...
  buscarFormasPagoBeneficios(estado) {
    this.limpiarResultado();
    this.cargando = true;
    let parametro = { empresa: LS.KEY_EMPRESA_SELECT, estado: estado };
    this.filasTiempo.iniciarContador();
    this.formaPagoBeneficiosService.listarFormasPagoBeneficios(parametro, this, LS.KEY_EMPRESA_SELECT);
  }

  despuesDeListarFormasPagoBeneficios(data) {
    this.filasTiempo.finalizarContador();
    this.cargando = false;
    this.listadoFormasPagoBeneficio = data;
  }
  ...
  insertarFormaPago(frmDatos) {
    ...
    if (this.validarAntesDeEnviar(frmDatos)) {
      ...
      let parametro = {
        rhFormaPagoBeneficioSocialTO: this.formaPago
      }
      this.formaPagoBeneficiosService.guardarFormasPagoBeneficios(parametro, this, LS.KEY_EMPRESA_SELECT);
    }
  }

  despuesDeGuardarFormasPagoBeneficios(data) {
    this.formaPago = data.extraInfo ? data.extraInfo : this.formaPago;
    this.toastr.info(data.operacionMensaje, LS.TAG_AVISO);
    this.listadoFormasPagoBeneficio.unshift(new RhListaFormaPagoBeneficioSocialTO(this.formaPago));
    this.gridApi ? this.gridApi.updateRowData({ add: [new RhListaFormaPagoBeneficioSocialTO(this.formaPago)], addIndex: 0 }) : null;
    this.vistaFormulario = false;
    this.cargando = false;
  }

  despuesDeAccionFormasPagoBeneficios(data) {
    if (data) {
      this.toastr.info(data.operacionMensaje, LS.TAG_AVISO);
      switch (this.accion) {
        case LS.ACCION_ACTIVAR:
        case LS.ACCION_INACTIVAR:
          var indexTemp = this.listadoFormasPagoBeneficio.findIndex(item => item.fpSecuencial === this.objetoSeleccionado.fpSecuencial);
          this.listadoFormasPagoBeneficio[indexTemp] = this.objetoSeleccionado;
          this.gridApi.updateRowData({ update: [this.objetoSeleccionado] });
          break;
        case LS.ACCION_EDITAR:
          var indexTemp = this.listadoFormasPagoBeneficio.findIndex(item => item.fpSecuencial === this.objetoSeleccionado.fpSecuencial);
          let listaTemporal = [... this.listadoFormasPagoBeneficio];
          listaTemporal[indexTemp] = new RhListaFormaPagoBeneficioSocialTO(this.formaPago);;
          this.listadoFormasPagoBeneficio = listaTemporal;
          this.refreshGrid();
          break;
        case LS.ACCION_ELIMINAR:
          var indexTemp = this.listadoFormasPagoBeneficio.findIndex(item => item.fpSecuencial === this.objetoSeleccionado.fpSecuencial);
          this.listadoFormasPagoBeneficio.splice(indexTemp, 1);
    ...
    if (this.utilService.validarKeyBuscar(event.keyCode) && this.formaPago.ctaCodigo) {
      ...
      modalRef.result.then((result) => {
        if (result) {
          ...
          this.formaPago.fpDetalle = this.formaPago.fpDetalle ? this.formaPago.fpDetalle.toUpperCase() : this.formaPago.fpDetalle;
  ...
  focusCodigo() {
    let element = document.getElementById('fpCodigoMinisterial');
    element ? element.focus() : null;
  }
  ...
  //#region [R3] [AG-GRID] 
  iniciarAgGrid() {
    this.columnDefs = this.formaPagoBeneficiosService.generarColumnas();
